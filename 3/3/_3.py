...
class TwitterClient(object): 
    ...
    def get_tweets(self, query,id, count,lang='en'): 
        ''' 
        ...
        try: 
            ...
            fetched_tweets = self.api.search(q = 'to:'+ query, since_id = id, result_type='recent', timeout=999999, count = count,lang='en')              
...
def main(): 
    ...
    # calling function to get tweets 
            ...
            id= input("Enter since_id to search for: ")
            count = input("Enter count to search for: ")

            replies = api.get_tweets(query, id, count, lang = 'en') 
    ...
    # picking positive tweets from tweets 
            ptweets = [tweet for tweet in replies if tweet['sentiment'] == 'positive'] 
    # percentage of positive tweets 
            print("Positive tweets percentage: {} %".format(100*len(ptweets)/len(replies))) 
            time.sleep(5)
    # picking negative tweets from tweets 
            ntweets = [tweet for tweet in replies if tweet['sentiment'] == 'negative'] 
    # percentage of negative tweets 
            print("Negative tweets percentage: {} %".format(100*len(ntweets)/len(replies))) 
            time.sleep(5)
    # percentage of neutral tweets 
            nuatral = (len(replies) - len(ntweets))- len(ptweets)
            print("Neutral tweets percentage: {} % ".format(100*(len(replies) - len(ntweets)- len(ptweets))/len(replies))) 
            time.sleep(5)
    # printing  positive tweets 
            print("\n\nPositive tweets:") 
            for tweet in ptweets: 
                print("Replies: ",tweet['text']) 
